...
void loop() {
  ...
  if (Serial.available()) {
	...
	if(byteRead == 49){             // keyboard key "1" - up freq
		rx = rx + increment;
		}
	if(byteRead == 50){		// keyboard key 2 - down freq
		rx = rx - increment;
		}
	if(byteRead == 51){		// keyboard key 3 - up increment
		setincrement();
		}
	if(byteRead == 52){		// keyboard key 4 - print VFO state
...
ISR(PCINT2_vect) {
  ...
  if (result) {    
    if (result == DIR_CW){rx=rx+increment;}
      ...
      if (rx >=72000000){rx=rx2;}; // UPPER VFO LIMIT 
